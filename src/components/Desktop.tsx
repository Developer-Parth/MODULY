...
import defaultWallpaper from '@/assets/images/images.jpeg';
    ...
    const handleMenuAction = (e: CustomEvent) => {
      ...
      switch (action) {
        ...
        case 'change-wallpaper':
          ...
          window.dispatchEvent(new CustomEvent('moduly-open-settings-section', { detail: 'wallpapers' }));
          onOpenApp('settings');
          break;
      }
    };
  ...
  return (
    <ContextMenu>
      <ContextMenuTrigger asChild disabled={selectedIcons.size > 0 && !selectionBox}>
        ...
        >
          <div className="absolute inset-0 pointer-events-none bg-[radial-gradient(circle_at_20%_15%,rgba(255,138,0,0.28),transparent_24%),radial-gradient(circle_at_80%_75%,rgba(31,191,117,0.22),transparent_28%),linear-gradient(135deg,rgba(0,0,0,0.26),rgba(0,0,0,0.58))]" />
          <div className="absolute inset-0 pointer-events-none opacity-[0.08] bg-[linear-gradient(90deg,white_1px,transparent_1px),linear-gradient(0deg,white_1px,transparent_1px)] bg-[size:44px_44px]" />

          {/* Selection Box */}
          {selectionBox && (
            <div
              className="absolute border border-orange-300/70 bg-orange-500/20 z-50 pointer-events-none rounded-xl shadow-[0_0_30px_rgba(255,138,0,0.25)]"
              style={{
                left: Math.min(selectionBox.start.x, selectionBox.current.x),
                top: Math.min(selectionBox.start.y, selectionBox.current.y),
...
const DesktopIconItem = memo(forwardRef<HTMLDivElement, DesktopIconItemProps>(function DesktopIconItem({
  ...
  return (
    <div
      ...
      onDragEnd={onDragEnd}
      className={`absolute flex flex-col items-center gap-1 p-2 rounded-[18px] cursor-pointer select-none 
      ${(!reduceMotion && !dragging) ? 'transition-all duration-150 ease-out' : ''} 
      ${selected ? 'bg-orange-500/20 backdrop-blur-md ring-2 ring-orange-300/60 shadow-[0_0_28px_rgba(255,138,0,0.28)]' : 'hover:bg-white/10 hover:backdrop-blur-sm'} ${props.className || ''}`}
      style={{
        left: position.x,
        top: position.y,
        width: '100px',
    ...
    >
      <div className={`relative w-14 h-14 mb-1 rounded-2xl ${!disableShadows ? 'drop-shadow-[0_10px_18px_rgba(0,0,0,0.45)]' : ''}`}>
        <FileIcon
          name={icon.name}
          type={icon.type === 'folder' ? 'directory' : 'file'}
          accentColor={accentColor}
      ...
      <div className={`text-[11px] leading-tight text-white text-center px-2 py-1 rounded-full bg-black/35 border border-white/10
