...
function MenuBarComponent({ focusedApp, onOpenApp }: MenuBarProps) {
  ...
  return (
    <div className="absolute top-2 left-0 right-0 flex justify-center z-[9999] pointer-events-none">
      <div
        className={cn(
          "h-10 px-4 rounded-full border border-white/10 flex items-center justify-between pointer-events-auto transition-all",
          !disableShadows && "shadow-2xl shadow-black/20"
        )}
        style={{ 
      ...
      >
        ...
        <div className="flex items-center space-x-4">
          <Menubar className="border-none bg-transparent h-auto p-0 space-x-1">
            <MenubarMenu>
              <MenubarTrigger
                className="bg-white/5 focus:bg-white/15 data-[state=open]:bg-white/15 px-3 py-1 h-7 rounded-full cursor-pointer transition-colors border border-white/5"
              ...
              <MenubarContent
                className={cn("border-white/10 text-white min-w-56 p-1 rounded-2xl z-[10000] mt-2", !disableShadows ? "shadow-2xl" : "shadow-none")}
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <MenubarItem onClick={() => {
                      ...
                      window.dispatchEvent(new CustomEvent('moduly-open-settings-section', { detail: 'about' }));
                      onOpenApp?.('settings');
                    }}>
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <MenubarItem
                      ...
                      className={cn(
                        "text-red-400 focus:text-red-400 focus:bg-red-500/20 rounded-md transition-colors",
                        panicConfirm && "bg-red-500/20"
                      )}
                    >
            ...
            <MenubarMenu>
              <MenubarTrigger className="font-bold text-[13px] px-3 data-[state=open]:text-white focus:text-white rounded-full h-7 items-center flex transition-colors hover:text-white/80 bg-white/0 hover:bg-white/10 data-[state=open]:bg-white/15">
              ...
              >
                ...
                <MenubarSeparator className="bg-white/10" />
                <MenubarItem className="text-orange-400 focus:text-orange-400 focus:bg-orange-500/20">{t('menubar.app.quitApp', { appName: activeApp?.nameKey ? t(activeApp.nameKey) : appConfig.name })}</MenubarItem>
              </MenubarContent>
            </MenubarMenu>
            ...
            {menuLabels.map((menu) => (
              <MenubarMenu key={menu}>
                <MenubarTrigger
                  className={cn(
                    "px-3 text-[13px] font-medium h-7 items-center flex transition-colors rounded-full",
                    "text-white/80 hover:text-white data-[state=open]:text-white focus:text-white",
                    "hover:bg-white/10 data-[state=open]:bg-white/15 bg-white/0"
                  )}
                  onClick={() => {
                ...
                <MenubarContent
                  className={cn("border-white/10 text-white min-w-48 p-1 rounded-2xl z-[10000] mt-2", !disableShadows ? "shadow-2xl" : "shadow-none")}
                  style={{ background: "rgba(0,0,0,0.4)", backdropFilter: "blur(24px)" }}
                >
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="text-white text-[13px] font-medium flex items-center gap-2 hover:bg-white/10 px-3 py-1 rounded-full transition-colors active:scale-95"
            title={timeMode === 'server' ? t('menubar.system.serverTime') : t('menubar.system.localTime')}
          >
            <span className="opacity-70">{currentDate}</span>
            <span className="font-semibold tracking-wide">{currentTime}</span>
          </button>
        </div>
      </div>
    </div>
