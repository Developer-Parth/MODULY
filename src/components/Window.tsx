...
function WindowComponent({
  ...
  const minimizeTarget = useMemo(() => {
    ...
    const target = dockCenter || {
      x: typeof globalThis !== 'undefined' ? globalThis.innerWidth / 2 : 500,
      y: typeof globalThis !== 'undefined' ? globalThis.innerHeight - 48 : 500
    };
    ...
    >
      <div
        className={cn(
          ...
          !disableShadows && isFocused && gpuEnabled && "shadow-2xl shadow-black/50",
          !isFocused && !window.isMinimized && "brightness-75 saturate-[0.8]",
          isDragging &&
            !disableShadows &&
            gpuEnabled &&
        ...
        style={{
          ...
          background: isFocused ? "rgba(15, 15, 15, 0.65)" : "rgba(15, 15, 15, 0.85)", 
          opacity: window.isMinimized ? 0 : 1,
          transform: getTransform(),
          backdropFilter: (blurEnabled && gpuEnabled)
            ? isDragging
              ? "blur(40px)"
              : "blur(32px)"
            : "none",
          WebkitBackdropFilter: (blurEnabled && gpuEnabled)
            ? isDragging
              ? "blur(40px)"
              : "blur(32px)"
            : "none",
          boxShadow: isFocused ? "inset 0 1px 1px rgba(255,255,255,0.1)" : undefined
        }}
      >
        {/* Title Bar */}
        <div
          className={cn(
            "window-title-bar h-12 border-b border-white/5 flex items-center justify-between px-4 cursor-move select-none shrink-0 relative group",
          )}
        ...
        >
          ...
          >
            ...
            <button
              className="w-3.5 h-3.5 rounded-full bg-[#ff5f56] hover:bg-[#ff5f56]/80 active:bg-[#ff5f56]/60 transition-colors border border-black/10 flex items-center justify-center overflow-hidden"
              onClick={handleClose}
            >
              <div className="opacity-0 group-hover:opacity-100 transition-opacity">
                <svg width="8" height="8" viewBox="0 0 8 8" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M1 1L7 7M7 1L1 7" stroke="rgba(0,0,0,0.5)" strokeWidth="1.2" strokeLinecap="round"/>
                </svg>
              </div>
            </button>
            <button
              className="w-3.5 h-3.5 rounded-full bg-[#ffbd2e] hover:bg-[#ffbd2e]/80 active:bg-[#ffbd2e]/60 transition-colors border border-black/10 flex items-center justify-center overflow-hidden"
              onClick={onMinimize}
            >
              <div className="opacity-0 group-hover:opacity-100 transition-opacity">
                <svg width="8" height="8" viewBox="0 0 8 8" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <button
              className="w-3.5 h-3.5 rounded-full bg-[#27c93f] hover:bg-[#27c93f]/80 active:bg-[#27c93f]/60 transition-colors border border-black/10 flex items-center justify-center overflow-hidden"
              onClick={onMaximize}
            >
              <div className="opacity-0 group-hover:opacity-100 transition-opacity">
                <svg width="8" height="8" viewBox="0 0 8 8" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M3 1h4v4M1 7l6-6" stroke="rgba(0,0,0,0.5)" strokeWidth="1.2" strokeLinecap="round"/>
                </svg>
              </div>
            </button>
          </div>

          <div className="absolute left-1/2 -translate-x-1/2 text-sm text-white/85 pointer-events-none font-semibold tracking-wide">
            {window.title}
          </div>
