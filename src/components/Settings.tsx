...
export function Settings({ owner }: { owner?: string }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener('moduly-open-settings-section', handleOpenSection as EventListener);
    return () => {
      window.removeEventListener('moduly-open-settings-section', handleOpenSection as EventListener);
    };
  }, [setActiveSection]);
  const {
    accentColor,
